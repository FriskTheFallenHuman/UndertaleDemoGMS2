...
if (text_part[0] == commands[command_1.create]) {
	...
	#endregion
} else if (text_part[0] == commands[command_1.undertale]) {
	#region undertale
		var parse_parts = array_length_1d(text_part)

		if (parse_parts == 1) { // object doesnt exist 
			temp_post_message = "Undertale must have sub commands"
			event_user(1); // post message to the history 
		} else {
			if (text_part[1] == "give_lv") {
				#region give_lv
					scr_levelup()
					sound_play(snd_levelup)
					command_done = true
				#endregion
			} else if (text_part[1] == "give_gold") {
				#region give_gold
				if (array_length_1d(text_part) < 3) {
					temp_post_message = "Please provide an amount of GOLD"
					event_user(1); // post message to the history 
				} else {
					var gold = real(text_part[2])
					if (string(gold) < 0) {
						temp_post_message = "Please provide a valid amount of GOLD"
						event_user(1); // post message to the history 
					} else {
				        global.gold += gold
				        sound_play(snd_item)
				...
				#endregion
			} else if (text_part[1] == "give_item") {
				#region give_item
				if (array_length_1d(text_part) < 3) {
					temp_post_message = "Please provide an item"
					event_user(1); // post message to the history 
				} else {
					var itemid = real(text_part[2])
					if (string(itemid) < 0) {
						temp_post_message = "Please provide a valid item"
						event_user(1); // post message to the history 
					} else {
				        scr_itemget(itemid)
				        sound_play(snd_item)
				...
				#endregion
			} else if (text_part[1] == "encounter") {
				#region encounter
				if (array_length_1d(text_part) < 3) {
					temp_post_message = "Please provide a encounter"
					event_user(1); // post message to the history 
				} else {
					var encounterid = real(text_part[2])
					if (string(encounterid) == "") {
						temp_post_message = "Please provide a valid encounter"
						event_user(1); // post message to the history 
					} else {
					    global.battlegroup = encounterid
					    global.border = 0
	...
	#region game
		...
		if (parse_parts == 1) { // object doesnt exist 
			...
			if (text_part[1] == "restart") {
				...
				command_done = true
				#endregion
			} else if (text_part[1] == "quit") {
				#region quit game
				game_end()
				command_done = true
				#endregion
			} else if (text_part[1] == "save") {
				#region save game
				scr_save()
				command_done = true
				#endregion
			} else if (text_part[1] == "tempsave") {
				#region save temporaly game
				scr_tempsave()
				command_done = true
				#endregion
			} else if (text_part[1] == "load") {
				#region load game
				scr_load()
				command_done = true
				#endregion
			} else if (text_part[1] == "tempload") {
				#region save temporaly game
				scr_tempload()
				command_done = true
				#endregion
	...
	#region console
		...
		if (parse_parts == 1) { // object doesnt exist 
			...
			if (text_part[1] == "clear_screen") {
				...
				temp_post_message = "UNDERTALE GMS2 port by Krispy#Console made by @davesinhispants  -  davetech.co.uk"
				event_user(1); // post message to the history 
	...
	#region help
		...
		event_user(1); // post message to the history
		temp_post_message = "undertale [flag] [var to update] [new value]"
		event_user(1); // post message to the history 
	...
	temp_post_message = "Invalid Command"
	event_user(1); // post message to the history 
}
