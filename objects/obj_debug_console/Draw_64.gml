...
if (display_frame > 0) { // draw something 
	...
	for (var i = 0; i < 5; ++i) {
		var increment = i + 1
		draw_rectangle(history_x1+increment,history_y1+increment,history_x2+increment,history_y2_draw+increment,true)
	}
	...
	for (var i = 0; i < 5; ++i) {
		var increment = i + 1
	    draw_rectangle(main_x1+increment,main_y1_draw+increment,main_x2+increment,main_y2_draw+increment,true)
	}
	...
	if (display_frame >= display_frames) { // predictions can only be seen at full open 
		...
		for (var i = 0; i < 5; ++i) {
			var increment = i + 1
			draw_rectangle(predict_x1+increment,predict_y1+increment,predict_x2+increment,predict_y2_draw+increment,true)
		}
	}
	...
	for (var i = 0; i < history_text_count; ++i) {
	    ...
	    if (history_text_shadow > 0) {
			...
			draw_text(history_text_x_draw+1,history_text_y_draw-(history_line_height*i)+1,string_hash_to_newline(history_text[i]))
		}
		...
		draw_text(history_text_x_draw,history_text_y_draw-(history_line_height*i),string_hash_to_newline(history_text[i]))
	}
